...
- **Delta freshness on large buckets** — discovery abandoned any folder wider than two 1,000-key pages, so a bucket with 2,500 interval prefixes per datasource truncated on every delta and never certified its freshness. The page budget is now 3; a new partition beyond page two is now discovered and indexed by a delta.
- **Bounded listing concurrency** — the crawler's listing pools were per bucket and multiplied, so a 19-bucket / 15.5M-object deployment exhausted a 1 GiB pod. One budget (`LIST_CONCURRENCY`, default 16) now covers every background-crawler listing path across all buckets and keeps concurrent lists inside the connection pool. Interactive browsing stays outside it, so the UI is unaffected while a crawl runs.
- **Fixes** — image previews load again (#27: the CSP did not name the S3 endpoint, so presigned URLs were blocked); LDAP is selectable on the login form (#35); local accounts can change their own password from the header (#29), and a wrong current password shows the error instead of signing you out; the password option is hidden from S3-key sessions, which have no local account; Cloudflare R2 setup names the Admin token tier that `ListBuckets` requires (#44).
- **Helm** — the default `persistence.size` is now 30Gi. A 1M-object index costs 1.0-1.4 GB including search, so 15.5M deep keys measures about 21 GB, and crawls stop below `MIN_FREE_DISK_PCT` free. Existing PVCs keep their size.

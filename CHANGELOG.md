...
The first boot on an index written by 3.6.0 repairs every bucket's search index (legacy indexes fail the new consistency check) — one at a time (`FTS_REBUILD_CONCURRENCY`, default 1), so a 22-bucket deployment is searchable bucket by bucket over the first minutes. Full reconciles of seeded buckets are spread across their interval instead of all falling due together. Rehearsed on the production-shaped lab (19 buckets, 15.5M objects, 1 GiB pod, 40 ms list latency): in-place upgrade in 11 s, then 75 minutes of sampling with **zero restarts and zero OOM kills**. All 19 buckets reached provider truth (15,500,000 of 15,500,000), with every search index current at gate completion. (Generations move continuously: a full reconcile bumps the catalogue generation when it starts and stamps the search marker when it finishes, so a point-in-time sample during rolling reconciles will always show some buckets mid-cycle. Deltas keep the index current through SQLite triggers and do not bump the generation.) Peak anonymous memory 605 MiB, against 888 MiB on the pre-fix candidate that was OOM-killed on the same workload. Peak working set reached 987 MiB, which is above the 800 MiB target but is dominated by reclaimable page cache from reading 15.5M rows: the cgroup reported `oom=0`, `oom_kill=0` and about 6.4 s of cumulative memory-pressure stall across the run.

The 3.6.0 baseline in that rehearsal was itself OOM-killed and left `seg-main` marked complete holding 1.32M of 9.9M objects. 3.7.0 repaired it to the full 9.9M. This therefore validates recovery from a damaged 3.6.0 index, not an upgrade from a fully indexed 3.6.0 baseline — 3.6.0 could not build one on this workload.
...
Measured on an unchanged bucket (see `benchmark/prodlab/RESULTS.md`, Runs 11–12): a 1M-object reconcile writes 31 MB instead of 1,101 MB and rewrites 0 rows instead of 1,000,000; a 9.9M-object reconcile writes 36 MB with a 326 MB memory peak where 3.6.0 was OOM-killed at 1 GiB. Both figures are for one bucket in isolation; the peak for a whole multi-bucket deployment is a separate measurement and is not claimed here.


**Upgrade note (Helm):** the default `persistence.size` moved from 5Gi to 30Gi (a 1M-object index costs 1.0-1.4 GB including search, so 15.5M deep keys measures about 21 GB, and crawls stop below `MIN_FREE_DISK_PCT` free). An existing release's PVC is patched in place, which only works when the StorageClass allows volume expansion; otherwise pass your current size (`--set persistence.size=5Gi`). The PVC now carries `helm.sh/resource-policy: keep`, so `helm uninstall` no longer deletes the index.

Crawler correctness (Core program, Phase A — SAE-63).

### Fixed

- **Delta discovery can page past 2,000 children, so large buckets certify their freshness again.** A folder was abandoned as oversized after two 1,000-key pages. A bucket whose datasources hold 2,500 interval prefixes therefore truncated its discovery walk on *every* delta, reported `discovery:truncated`, and never advanced its freshness timestamp — deterministically, not intermittently. The budget (`DELTA_NODE_MAX_PAGES`) is now 3 pages; a genuinely wider folder still degrades rather than being walked in full. Verified end to end: a brand-new partition under a 2,501st interval is discovered and indexed by a delta, with no full crawl involved.
- **Background-crawler listing concurrency is bounded across all buckets, not per bucket.** The listing pools were sized per bucket and multiplied: twelve concurrent bucket tasks each opening sixteen listing threads is 192 listers in flight, and the memory lives in their buffers. A 19-bucket / 15.5M-object deployment on a 1 GiB pod was OOM-killed with repeated restarts and a persistently overflowing connection pool. One budget (`LIST_CONCURRENCY`, default 16, clamped to `S3_MAX_POOL_CONNECTIONS`) now covers **every background-crawler listing path** — root discovery, sub-prefix expansion, initial crawl, reconcile, simple whole-bucket crawl, delta discovery and delta targets. Interactive browsing is deliberately outside the budget so the UI stays responsive while a crawl runs. Measured on the same rehearsal: the repair of `seg-main` from 1.32M to 9.9M objects, running alongside reconciles of the other 18 buckets, completed in about 10 minutes, and a steady-state delta of the 9.9M-object bucket takes 17-27 s.
- **Image previews load again.** The Content-Security-Policy did not name the S3 endpoint, so a presigned preview URL was blocked and the viewer showed a broken image (#27). The endpoint's origin is now allowed for images, media and frames. The browser test asserts the image actually decoded rather than accepting the failure placeholder, so a regression here fails CI.
- **LDAP is selectable on the login form again** (#35), and users with a local account can change their own password from the header (#29). Both are covered by browser tests in the required lane; the LDAP one proves the form reaches the LDAP endpoint rather than silently falling back to local auth.
- **A wrong current password no longer signs you out.** Every 401 was treated as an expired session, so mistyping your current password while changing it returned you to the login screen instead of showing the error.
- **The password option is hidden from sessions that authenticate with S3 keys.** Such a session has no local account, so a missing `auth_source` was read as "local" and offered a password change that could only fail.
- **Cloudflare R2 setup names the right token tier** (#44). Sairo discovers buckets with `ListBuckets`, which only R2's Admin tiers allow; the guide recommended an Object token, so following it produced a bare `AccessDenied` and no buckets. A denied listing now explains what to change instead of surfacing the raw provider error.
...
- Helm: the data PVC carries `helm.sh/resource-policy: keep` (`persistence.keep: true`) so `helm uninstall`/reinstall keeps the index; default `persistence.size` is now **30Gi** (1.0-1.4 GB per 1M objects including the search index, plus the free margin the disk guard needs). Existing PVCs keep their size.
